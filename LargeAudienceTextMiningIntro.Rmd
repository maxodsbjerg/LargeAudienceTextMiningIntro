...
council %>% 
  mutate(text = str_replace_all(text, pattern = "Aarhus Byraad", "aarhus_byraad")) %>%
  mutate(text = str_replace_all(text, pattern = "Aarhus Amt", "aarhus_amt")) %>% 
  mutate(text = str_replace_all(text, pattern = "Aarhus Museum", "aarhus_museum"))-> council_clean
```
...
```{r}
council_clean %>% 
  mutate(aar = year(date_of_meeting)) -> council_clean
```
...
council_clean %>% 
  ...
  select(word, everything()) -> council_tidy
```
...
council_tidy %>% 
  anti_join(stopord, by = "word") %>% 
  anti_join(council_stopwords, by = "word") -> council_tidy_no_stopwords
```
...
council_tidy_no_stopwords %>% 
  ...
  geom_col() +
  facet_wrap(~aar, ncol = 4, scales = "free_y") +
  coord_flip()
```
